<?php
    ...
    $query_ttd = mysqli_query($koneksi, 'UPDATE tbl_tanda_tangan SET STATUS="diterima", status_notif="diterima", catatan="' . $catatan . '", tgl_proses = "' . $tgl_proses . '" WHERE id_surat="' . $id_surat . '" AND id_user ="' . $id_user . '"');	
    ...
    if($pangkat == "guru"){
        ...
        while ($data_pangkat = mysqli_fetch_array($cek_pangkat)) {
            ...
            $query_ttd = mysqli_query($koneksi, 'UPDATE tbl_tanda_tangan SET STATUS="cek", status_notif="diajukan" WHERE id_surat="' . $id_surat . '" AND id_user ="' . $id_user_p . '"');	
        }	
