<?php 
    ...
    $jenis_surat = 'surat_pemberitahuan';
    ...
    $query = mysqli_query($koneksi, 'insert into tbl_surat set jenis="'.$jenis_surat.'", hari="'.$hari.'", tgl_pelaksanaan="'.$new_tanggal_pelaksanaan.'", waktu="'.$waktu.'", tempat="'.$tempat.'", kepada="'.$kepada. '", alamat="' . $alamat . '", perihal="'.$perihal.'", keterangan="'.$dalam_rangka. '", tgl_pembuatan="'.$new_tanggal_pembuatan.'", id_pemohon = "'.$_SESSION['id_user'].'", hapus = "n"');
    ...
    if($filename != null || $filename != ''){
        $xx = $rand.'_surat_pemmberitahuan'.$filename;
        move_uploaded_file($_FILES['file_lampiran']['tmp_name'], '../../upload/'.$rand.'_surat_pemmberitahuan'.$filename);
